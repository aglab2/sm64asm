...
.area 0x80
	...
	SW A2, 0x0030 (SP) ; 0x30 - rom end
	;SW R0, 0x0024 (SP) ; 0x24 - decompressed mio0
	...
	ADDIU A1, R0, 0x0001
	SW V0, 0x001C (SP) ; 0x1C - alloced temp mio0
	; Counting ram size is not needed for static
	;ADDIU T2, V0, 0x0004
	;SW T2, 0x0018 (SP) ; 0x18 - ptr to ram size
	;LW T3, 0x001C (SP)
	;BEQZ T3, fail
	;NOP
	LW A0, 0x001C (SP)
	...
	LW A2, 0x0030 (SP)
	;LW T4, 0x0018 (SP)
	; ===== DYNAMIC MEM ALLOC START =====
	;OR A1, R0, R0
	;JAL 0x80278120 ; malloc(ram size, 0)
	;LW A0, 0x0000 (T4)
	;SW V0, 0x0024 (SP) ; 0x24 - decompressed mio0 ram
	;LW T5, 0x0024 (SP)
	;BEQZ T5, 0x8027888C
	;NOP
	; ===== DYNAMIC MEM ALLOC END =====
	LW A0, 0x001C (SP)
	JAL 0x8027F4E0 ; unpack(mio0 packed, mio0 unpacked)
	LW A1, 0x0024 (SP)
	; ===== BANK SETTING START =====
	;LW A0, 0x0028 (SP) ; bank warning
	;JAL 0x80277EE0 ; bankfunc(bank, ptr)
	;LW A1, 0x0024 (SP)
	; ===== BANK SETTING END =====
